...
def registrar_usuario(rest: Restaurante, archivo_servicio: ArchivoServicio) -> None:
    print("\nRegistro de usuario:")
    ...
    if rest.registrar_usuario(usuario):
        archivo_servicio.guardar_usuarios(rest.obtener_usuarios())
...
def actualizar_producto(rest: Restaurante, archivo_servicio: ArchivoServicio) -> None:
    ...
    stock_input = solicitar_input(f"Nuevo stock [{producto.stock}]: ")
...
def eliminar_usuario(rest: Restaurante, archivo_servicio: ArchivoServicio) -> None:
...
def vender_producto(rest: Restaurante, archivo_servicio: ArchivoServicio) -> None:
    print("\nVender producto:")
    identificacion = solicitar_input("Identificación del usuario: ")
    codigo = solicitar_input("Código del producto: ")
    try:
        cantidad = int(solicitar_input("Cantidad: "))
    except ValueError:
        print("Cantidad inválida. Operación cancelada.")
        return

    resultado = rest.vender_producto(codigo, identificacion, cantidad)
    if resultado:
        archivo_servicio.guardar_ventas(rest.obtener_ventas())
        archivo_servicio.guardar_productos(rest.obtener_productos())
        print("Venta registrada correctamente.")
    else:
        print("No se pudo realizar la venta. Verifica usuario/producto/cantidad/stock.")


def consultar_ventas_usuario(rest: Restaurante) -> None:
    identificacion = solicitar_input("Identificación del usuario a consultar: ")
    ventas = rest.ventas_por_usuario(identificacion)
    if not ventas:
        print("No se encontraron ventas para ese usuario.")
        return
    print(f"Ventas del usuario {identificacion}:")
    for venta in ventas:
        producto = rest.obtener_producto(venta.producto_codigo)
        nombre_producto = producto.nombre if producto is not None else "(producto eliminado)"
        print(f"- Código: {venta.producto_codigo} | Nombre: {nombre_producto} | Cantidad: {venta.cantidad}")
...
def main() -> None:
    ...
    rest = Restaurante(productos=productos_iniciales, usuarios=usuarios_iniciales, ventas=ventas_iniciales)
    ...
    menu_actions: Dict[str, Callable[[Restaurante, ArchivoServicio], None]] = {
        ...
        "6": lambda r, a: registrar_usuario(r, a),
        "7": lambda r, a: eliminar_usuario(r, a),
        "8": lambda r, a: listar_usuarios(r),
        "9": lambda r, a: mostrar_categorias(r),
        "10": lambda r, a: vender_producto(r, a),
        "11": lambda r, a: consultar_ventas_usuario(r),
    }
